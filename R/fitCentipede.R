#' The main function 
#' 
#' Require at least two input, list of matrix of read counts, 
#' covariate data matrix. 
#' See \url{https://github.com/slowkow/CENTIPEDE.tutorial} for details about 
#' how to prepare Xlist object. The function \code{centipede_data()} was
#' designed to generate list of matrix. 
#' 
...
#' @param Xlist list; List of data matrices
#' @param Y matrix; Covariate data matrix
#' @param LambdaParList list; not required
#' @param BetaLogit list; not required
#' @param NegBinParList list; not required
#' @param sweeps integer; default 50
#' @param DampLambda float; default 0.0
#' @param DampNegBin float; default 0.0
#' @param TrimP float; default 0.0001 
#' @param NRiter integer; default 5
#'
#' @return 
fitCentipede <- function(
  Xlist, Y, LambdaParList, BetaLogit, NegBinParList, sweeps = 50, 
  DampLambda = 0.0, DampNegBin = 0.0, TrimP = 0.0001, NRiter = 5
) {
  ...
  ## Assuming Dnase is the first one...
  Ez <- (Rlist[[1]] > quantile(Rlist[[1]], 0.9)) + 0.0 
  
  ## BetaLogit initialization, ....  
  if (missing(BetaLogit)) {
    BetaLogit  <- matrix(0, K, 1)
    BetaFit    <- optim(BetaLogit, 
                        fLogit, 
                        gLogit, 
                        Y  = Y, 
                        Ez = Ez, 
  ...
  if (missing(NegBinParList)) {
    NegBinParList <- lapply(Rlist, 
                            initNegBinParamsMixture, 
                            Ez = Ez, 
                            DampNegBin = DampNegBin)
    NegBinParList <- mapply(compNegBinParamsMixture, 
                            Rlist, 
                            NegBinParList, 
                            MoreArgs = list(Ez = Ez, DampNegBin = DampNegBin))
  }
  ...
  LogLikNoMix <- sum(mapply(compLogLik1, 
                            Xlist, 
                            LambdaParNoMixList, 
                            NegBinParNoMixList, 
                            MoreArgs = list(TrimP      = TrimP, 
                                            DampNegBin = DampNegBin)))
  
  LogLikNoMixNoNegBin <- sum(mapply(compLogLik1, 
                                    Rlist, 
                                    LambdaParNoMixList,
                                    NegBinParNoMixList,
                                    MoreArgs = list(TrimP      = TrimP, 
  ...
  PriorLogRatio <- Y %*% BetaLogit
  NegBinLogRatio <- rowSums(mapply(compNegBinLogRatio, 
                                   Rlist, 
                                   NegBinParList,
                                   MoreArgs = list(TrimP=TrimP, 
                                                   DampNegBin=DampNegBin)))
  MultiNomLogRatio <- rowSums(mapply(compMultiNomLogRatio, 
                                     Xlist, 
                                     LambdaParList,
                                     MoreArgs = list(TrimP=TrimP)))
  ...
  LogLikEvo <- 0
  LogLik0 <- sum(mapply(compLogLik0, 
                        Rlist, 
                        NegBinParList,
                        Slist, 
                        MoreArgs=list(TrimP=TrimP,
                                      DampNegBin=DampNegBin))
  ) - sum(log(1+exp(Y %*% BetaLogit)))
  Aux <- log(1+exp(LogRatios))
  Aux[LogRatios>18] <- LogRatios[LogRatios>18] 
  ## makes the thing more stable if LogRatio very large! so exp(LogRatio) does not explode
  LogLikEvo <- sum(Aux) + LogLik0 
  ## + sum(log(1-myPi)) - L*lgamma(A0) + L*log(B0)*A0 + sum(lgamma(R+A0)) + log(1-B0)*sum(R) + MyLogLikConst 
  ##  - sum(R*log(S))
  ...
  while(IterateEM){
    ...
    LambdaParList <- lapply(Xlist, 
                            compLambda,
                            Ez = Ez,
                            DampLambda = DampLambda)
    ...
    Aux[LogRatios > 18] <- LogRatios[LogRatios > 18] 
    ## makes the thing more stable if LogRatio very large! so exp(LogRatio) does not explode
    LogLikEvo[i] <- sum(Aux) + LogLik0 
    ## + sum(log(1-myPi)) - L*lgamma(A0) + L*log(B0)*A0 + sum(lgamma(R+A0)) + log(1-B0)*sum(R) + MyLogLikConst 
    ##  - sum(R*log(S))
    
    ## Parameter Convergence monitoring..... 
    LambdaChange   <- sapply(mapply(function(i,j){abs(j-i)}, OldLambdas, 
                                    LambdaParList), max)
    NegBinChange   <- as.matrix(mapply(function(i,j){
      (as.data.frame(j) - as.data.frame(i))
      }, OldNegBinPars, NegBinParList))
    LogitChange    <- (BetaLogit - OldBetaLogit)
    MaxParamChange <- c(max(abs(LambdaChange)), max(abs(unlist(NegBinChange))),
                        max(abs(LogitChange)))
    str(MaxParamChange)
    ...
    if(i>1){
      cat("  LogLikComplete:", LogLikEvo[i], "Change:", 
          LogLikEvo[i]-LogLikEvo[i-1],
          " Max Parameter Change:",
          max(MaxParamChange),"\n")
      if((abs(LogLikEvo[i] - LogLikEvo[i-1]) < 0.02) && 
         (max(MaxParamChange) < 1E-3) ){
        ConvergedEM <- TRUE
      }
    }
  ...
  list(PostPr  = Ez, 
       NumIter = i,
       LogRatios      = LogRatios,
       PriorLogRatio  = PriorLogRatio,
       NegBinLogRatio = NegBinLogRatio,
       LambdaParList  = LambdaParList, 
       BetaLogit      = BetaLogit,
       PriorPr        = plogis(Y %*% BetaLogit),
       LogLikEvo      = LogLikEvo,
       LogLikEnd      = LogLikEvo[i],
       MultiNomLogRatio    = MultiNomLogRatio,
       NegBinParList       = NegBinParList, 
       ModelPartsVar       = cc, 
       ModelPartsCor       = dd,
       LogLikNoMix         = LogLikNoMix,
       LogLikNoMixNoNegBin = LogLikNoMixNoNegBin,
       NegBinParNoMixList  = NegBinParNoMixList, 
       LambdaParNoMixList  = LambdaParNoMixList) 
}
...
fNegBin2 <- function(
  ...
  logitB <- NegBinPar[2]
  if((A < 1E200) && (A > 1E-200)){
    LogLik <- -(sum( Ez * ( lgamma(R+A) - lgamma(A) + A * logitB - 
                              (R + A) * log(1 + exp(logitB)))))
  }else{
    LogLik <- 1E300
  }
  LogLik
...
initNegBinParamsMixture <- function(
  R, Ez, DampNegBin = 0.0
  ){
...
compNegBinParamsMixture <- function(
  R, NegBinPar, Ez, DampNegBin = 0.0, NRiter = 5
  ){
  #str(NegBinPar)
  ...
  if(DampNegBin < 1){
    ...
    NegBinPar$logitB0 <- NegBinPar$logA0 + log(sum(1 - Ez)) - 
      log(sum(R * (1 - Ez)))
    ##str(NegBinPar)
    ...
    NegBinPar0 <- optim(NegBinPar0, fNegBin2, gNegBin2, R=R, Ez=(1 - Ez),
                        method  = "L-BFGS-B",
                        lower   = c(-20, -20),
                        upper   = c(20, 20),
                        control = list(maxit = NRiter))
    NegBinPar1 <- optim(NegBinPar1, fNegBin2, gNegBin2, R=R, Ez=Ez, 
                        method  = "L-BFGS-B",
                        lower   = c(-20, -20),
                        upper   = c(20, 20),
                        control = list(maxit = NRiter))
...
compNegBinParams <- function(
  R, NegBinPar, NRiter = 5
  ){
  ...
  NegBinPar   <- optim(NegBinPar, fNegBin2, gNegBin2, R = R, Ez = 1, 
                       method  = "L-BFGS-B",
                       lower   = c(-20, -20),
                       upper   = c(20, 20),
                       control = list(maxit = NRiter))$par   
  ##str(NegBinPar)
  list(logitB = NegBinPar[2],
       logA   = NegBinPar[1])
}
...
compNegBinLogRatio <- function(
  R, NegBinPar, TrimP = 0.0001, DampNegBin = 0.0
  ){
  if(DampNegBin < 1){
    ...
    NegBinLogRatio <- lgamma(A0) - lgamma(R + A0) - lgamma(A1) + lgamma(R+A1) +
      A1 * logitB1 - (R + A1) * log(1 + exp(logitB1)) - 
      A0 * logitB0 + (R + A0) * log(1 + exp(logitB0))
...
compMultiNomLogRatio <- function(
  X, Lambda, TrimP = 0.0001
  ){
  S <- dim(X)[2]
  if(S > 1){
...
compLogLik0 <- function(
  R, NegBinPar, S, TrimP = 0.0001, DampNegBin = 0.0
  ){
  if(DampNegBin < 1){
    A0 <- exp(NegBinPar$logA0)
    logitB0 <- NegBinPar$logitB0
    LogLik0 <-  lgamma(R+A0) - lgamma(A0) + A0 * logitB0 - 
      (R+A0) * log(1 + exp(logitB0)) - R * log(S)
...
compLogLik1 <- function(
  X, Lambda, NegBinPar, TrimP = 0.0001, DampNegBin = 0.0
  ){
  S <- dim(X)[2]
  R <- rowSums(X)
  if(S > 1){
  ...
  if(DampNegBin < 1){
    ...
    NegBinLogLik <- lgamma(R + A) - lgamma(A) + A * logitB - 
      (R + A) * log(1 + exp(logitB))
  }else{
    NegBinLogLik <- 0.0
  }
